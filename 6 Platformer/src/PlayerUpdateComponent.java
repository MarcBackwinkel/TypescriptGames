...
class PlayerUpdateComponent implements UpdateComponent {
    ...
    public void update(long fps, Transform t, Transform playerTransform) {
        ...
        //Check if jump was triggered by the player and if player is NOT ALREADY
        //jumping or falling (because we don't want to jump in mid air)
        //If you want double jump (or tripe etc.) allow the jump when not grounded,
        //and count the number of nun-grounded jumps and disallow jump when your
        //preferred limit is reached
        if (pt.jumpTriggered() && !mIsJumping && pt.isGrounded()){
            SoundEngine.playJump();
            mIsJumping = true;
            pt.handlingJump();
            //Thanks for the notification, I'll take it from here
