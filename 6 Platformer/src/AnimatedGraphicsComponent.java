...
class AnimatedGraphicsComponent implements GraphicsComponent {
    ...
    public void initialize(Context context, GameObjectSpec spec,
        ...
        mAnimator = new Animator(objectSize.y, objectSize.x, spec.getNumFrames(),
                pixelsPerMetre);
        ...
        BitmapStore.addBitmap(context, mBitmapName,
                new PointF(totalWidth, objectSize.y), pixelsPerMetre, true);
    ...
    public void draw(Canvas canvas, Paint paint, Transform t, Camera cam) {
        ...
        //Get the section of bitmap to draw when an object is in motion OR
        //if it is an object with zero speed (like a fire tile)
        ...
        Rect screenCoordinates = cam.worldToScreen(t.getLocation().x,
                t.getLocation().y, t.getSize().x, t.getSize().y);

        if (t.getFacingRight()){
            canvas.drawBitmap(BitmapStore.getBitmap(mBitmapName),
                    mSectionToDraw, screenCoordinates, paint);
        } else {
