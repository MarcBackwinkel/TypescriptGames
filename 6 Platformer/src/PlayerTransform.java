...
class PlayerTransform extends Transform {
    ...
    //4 player specific colliders (in addition to the general player collider)
    ...
    PlayerTransform(float speed, float objectWidth, float objectHeight,
    ...
    public ArrayList<RectF> getColliders(){
        ...
        //specific colliders are not updated every frame to save time; so if they
        //are requested, they first must be updated
        updateColliders();
        return mColliders;
    }
    ...
    public void updateColliders(){
        ...
        mColliders.get(0).top = location.y + objectHeight - (objectHeight * TENTH);
        mColliders.get(0).right = location.x + objectWidth - (objectWidth * THIRD);
        //???? --> Book Position 11540
        mColliders.get(0).bottom = location.y + (objectHeight * FEET_PROTRUSION);
        ...
        mColliders.get(3).top = location.y + (objectHeight * FIFTH);
    ...
    //Called by movemenet component to let transform know that movement component is
    //aware jump was triggered
    void handlingJump(){
        mJumpTriggered = false;
    }

    //Used by movement component ot find out if jump has been triggered
