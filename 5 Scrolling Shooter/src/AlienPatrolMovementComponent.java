    ...
    public boolean move(long fps, Transform t, Transform playerTransform) {
        ...
        if (Math.abs(loc.x - playerLocation.x) < mSeeingDistance){
            if (playerTransform.getFacingRight() != t.getFacingRight()){
                ...
                horizontalSpeedAdjustmentRelativeToPlayer = speed *
        ...
        //Moving horizontally taking into account the speed modification
        if (t.headingLeft()){
            loc.x -= (speed + horizontalSpeedAdjustmentRelativeToPlayer) / fps;

            //Turn the ship around when it reaches the extent of its
            //horizontal patrol area
        ...
        //Vertical speed remains same, not affected by speed adjustment
        if (t.headingDown()){
            loc.y += (speed) / fps;
        ...
        //Shoot if the alien within a ships height above, below or in line with
        //the player? This could be a hit or a miss
        if (mShotRandom.nextInt(SHOT_CHANCE) == TAKE_SHOT){
            if (Math.abs(playerLocation.y - loc.y) < height){
                //is the alien facing the right direction and close enough to the
                //player
                if ((t.getFacingRight()
