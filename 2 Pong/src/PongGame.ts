...
class PongGame extends SurfaceView implements Runnable{
    ...
    //This method overrides the only method in the Runnable interface - run()
    //it is needed for handling a thread
    ...
    public void run() {
        ...
        //It is all handled by Android, the Thread Class and the Runnable Interface

        //mPlaying gives us finer control rather than just relying on the calls to run
        //mPlaying must be TRUE
        while (mPlaying){
            //What time is it now at the start of the loop?
            long frameStartTime = System.currentTimeMillis();

            //Provided the game is not paused call the update method
            if (!mPaused){
                update();

                //Now the bat and ball are in their new positions we can see
            ...
            //Make sure that timeThisFrame is at least 1 millisecond because accidentally
            //dividing by zero crashes the game
            if (timeThisFrame > 0){
    ...
    public void pause(){
        ...
        //Stopping the thread is not always instant
        mPlaying = false;
        try {
            //Stop the thread
            mGameThread.join();
    ...
    private void draw(){
        ...
        if (mOurHolder.getSurface().isValid()){
            ...
            //sends our mCanvas object for drawing to the screen and
